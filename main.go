	...
	for index, keyword := range inputKeywordsArray {
		...
		for _, word := range keywordArray {
			...
			if word != normMainKeyword {
				...
				log.Printf("Word %s is same as main keyword, skipped", word)
			}
		}
	}
	log.Printf("Workaround finished")
...
func PrintData(field *gtk.TextView, totalCountLabel *gtk.Label, resultCountLabel *gtk.Label, result chan resultData, mutex *sync.Mutex) {
	var data resultData
	data = <-result
...
func UpdateProgress(bar *gtk.ProgressBar, progress chan float64, mutex *sync.Mutex) {
	for {
		var parsent float64
		parsent = <-progress
		mutex.Lock()
...
func main() {
	...
	proccessButton.Connect("clicked", func() {
		...
		go PrintData(resultKeywordsField, totalCountLabel, resultCountLabel, resultChannel, &mutex)
		go UpdateProgress(progressBar, progressChannel, &mutex)
	})
